"""
Accuracy validation suite against a ground-truth JSON manifest using /voice.
"""
...
def _post_multipart(base_url: str, image_bytes: bytes, fields: dict) -> dict:
...
def run_validation(manifest_path: str, base_url: str) -> int:
    ...
    for item in teach_items:
        ...
        result = _post_multipart(base_url, img_bytes, {"request_type": "remember", "prompt": item["label"]})
        status = "[PASS]" if result.get("result", {}).get("success") else "[FAIL]"
    ...
    for test in scan_tests:
        ...
        img_bytes = _load_image_bytes(img_path)
        result = _post_multipart(base_url, img_bytes, {"request_type": "scan"})
