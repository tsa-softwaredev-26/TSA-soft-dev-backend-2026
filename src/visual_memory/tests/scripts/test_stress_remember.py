"""Stress tests for /voice remember burst and payload quality."""
...
def _remember_ok(color: tuple[int, int, int]) -> tuple[int, float]:
    ...
    resp = client.post(
        "/voice",
        data={"request_type": "remember", "prompt": "wallet", "image": (io.BytesIO(_img_bytes(color)), "sample.jpg")},
...
def _remember_missing_prompt() -> int:
    resp = client.post(
        "/voice",
        data={"request_type": "remember", "image": (io.BytesIO(_img_bytes((0, 0, 0))), "sample.jpg")},
        content_type="multipart/form-data",
    )
...
def test_remember_burst_and_invalid_mix():
    ...
    with ThreadPoolExecutor(max_workers=10) as ex:
        ok_results = list(ex.map(_remember_ok, colors * 20))
    ...
    append_recommendations([
        ...
        "- If failures spike, isolate multipart parsing and upload handling paths.",
    ])
    assert not failures
